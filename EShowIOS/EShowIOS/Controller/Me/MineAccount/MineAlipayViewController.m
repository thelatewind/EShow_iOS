...
@interface MineAlipayViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>{
    UITableView *tableV;
    NSArray *leftTitleArr;
    UITextField *phoneNumField;
    UITextField *verifyNumField;
    NSString *alipayNum;//微信号
    NSString *alipayName;//姓名
}
@property (strong, nonatomic) UIButton *captchaBtn;
@end

@implementation MineAlipayViewController
...
-(void)createData{
    leftTitleArr = @[@[@"支付宝",@"姓名"],@[@"手机号码",@"请输入验证码"]];
}
-(void)createTableView{
    ...
    tableV.tableFooterView = [self customFooterView];
    tableV.tableHeaderView = [self customHeaderView];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            if (alipayNum.length==0) {
                NSString *aliPayStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"alipayName"];
                if (aliPayStr.length) {
                    cell.detailTextLabel.text = aliPayStr;
                    cell.detailTextLabel.textColor = RGBColor(51, 51, 51);
                }
                else{
                    cell.detailTextLabel.text = @"请输入支付宝账号";
                    cell.detailTextLabel.textColor = [UIColor colorWithRed:0.33 green:0.72 blue:0.87 alpha:1];
                }
            }else{
                cell.detailTextLabel.text = alipayNum;
                cell.detailTextLabel.textColor = RGBColor(51, 51, 51);
            }
        }else if (indexPath.row==1) {
            if (alipayName.length==0) {
                cell.detailTextLabel.text = @"请输入支付宝姓名";
                cell.detailTextLabel.textColor = [UIColor colorWithRed:0.33 green:0.72 blue:0.87 alpha:1];
            }else{
                cell.detailTextLabel.text = alipayName;
                cell.detailTextLabel.textColor = RGBColor(51, 51, 51);
            }
        }
...
//        NSString *str = [MineAlipayViewController valiMobile:phoneNumField.text];
        if ([MineAlipayViewController valiMobile:phoneNumField.text]) {
            [self requestVerify];
        }else{
...
-(void)presentBtnClick{
    if (alipayNum.length==0) {
        [self.view makeToast:@"请输入支付宝账号" duration:1 position:@"center"];
    }else if (alipayName.length==0) {
        [self.view makeToast:@"请输入支付宝姓名" duration:1 position:@"center"];
        ...
        [[NetworkEngine sharedManager] request_Save_AlipayAccount_WithParams:@{} andBlock:^(id data, NSError *error) {
            
        }];
//        NSDictionary *dict = @{
//                               @"user.realname":alipayName,
//                               @"user.alipay":alipayNum,
...
//            NSDictionary *dic = (NSDictionary *)responseBody;
//            if([dic[@"status"] integerValue]==1){
//                [YUserDefaults setObject:alipayNum forKey:@"alipayName"];
//                if (self.backAlipayBlock) {
//                    self.backAlipayBlock(@"");
//                }
//                [self.view makeToast:dic[@"msg"] duration:1 position:@"center"];
